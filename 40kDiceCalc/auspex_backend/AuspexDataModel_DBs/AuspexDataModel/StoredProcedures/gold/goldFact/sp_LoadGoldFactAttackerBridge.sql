CREATE PROCEDURE [dbo].[sp_LoadGoldFactAttackerBridge] AS BEGIN
SET
    NOCOUNT ON;

INSERT INTO
    [dbo].[gold_fact_attacker_bridge] (
        runid,
        attacker_sk,
        attacker_status_sk,
        modifier_sk
...
FROM
    ...
    INNER JOIN [dbo].[gold_dim_attacker_profile] ap ON sa.attackername = ap.unitname
    ...
    INNER JOIN [dbo].[gold_dim_attacker_status] st ON sa.is_leader = st.is_leader
    AND ISNULL(sa.granted_keyword, '') = ISNULL(st.granted_keyword, '')
    INNER JOIN [dbo].[gold_dim_modifiers] m ON sa.mod_lethal = m.mod_lethal
    ...
    AND sa.mod_fish_for_crits = m.mod_fish_for_crits -- pevent inserting the same attacker for the same run
WHERE
    NOT EXISTS (
        select
            1
        from
            [dbo].[gold_fact_attacker_bridge] AS b
        WHERE
            b.runid = sa.runid
