CREATE PROCEDURE [dbo].[sp_LoadSilverSimulations] AS BEGIN
SET
    NOCOUNT ON;

-- 1. INSERT new simulations INTO silver_simulations
INSERT INTO
    [dbo].[silver_simulations] (
        runid,
        batchid,
        userid,
        [timestamp],
...
FROM
    ...
    CROSS APPLY OPENJSON(
        b.jsonpayload,
        '$.simulation_parameters.target_unit'
    ) WITH (
    ...
    LEFT JOIN [dbo].[silver_simulations] existing_s ON existing_s.runid = SessionData.run_id
WHERE
    existing_s.runid IS NULL;
