CREATE PROCEDURE [dbo].[sp_LoadSilverAttackers] AS BEGIN
SET
  NOCOUNT ON;

-- 2. INSERT new attackers INTO silver_attackers
INSERT INTO
  [dbo].[silver_attackers] (
    runid,
    unit_id,
    attackername,
    attackerfaction,
...
FROM
  ...
  CROSS APPLY OPENJSON(b.jsonpayload, '$.session_data') WITH (run_id uniqueidentifier '$.run_id') AS SessionData
  CROSS APPLY OPENJSON(
    b.jsonpayload,
    '$.simulation_parameters.attacker_units'
  ) WITH (
