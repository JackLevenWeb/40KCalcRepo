...
This application is a web-based probability calculator for tabletop simulation mechanics. It processes complex cascading probabilities via a waterfall simulation model, executing 250,000 iterations to output structured statistical distributions of damage, hits, wounds, and saves.
...
- Combinatorial Analysis: Features a permutation engine that generates Cartesian products of user-selected modifiers (inclusive, exclusive, and independent) to mathematically determine optimal weapon loadouts.
- Combined-Arms Simulation: Automatically detects attached Leader profiles and groups them with Bodyguard units to share keyword abilities during combined simulations.
...
- event-manager.js: Centralized event bus handling DOM listeners, CustomEvent broadcasting, and drag-and-drop mechanics.
...
- combinatorial-engine.js: Scrapes UI state and generates valid permutation arrays for modifier analysis.
- modifier-registry.js: Centralized configuration dictionary acting as a single source of truth for modifier properties, default values, and UI categories.
- fetchUnitStats.js: Handles OpenHammer API requests, search debouncing, and payload translation mapping.
- webWorker.js: Background thread handler for intensive simulation loops.
- classes/: Directory containing object-oriented models (`Unit.js`, `Weapon.js`, `Dice.js`) for isolated state management.
